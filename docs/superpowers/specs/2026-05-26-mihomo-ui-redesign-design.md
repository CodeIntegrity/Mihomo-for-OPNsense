...
| Q4 | 菜单与页面组织 | A 扁平多顶项 / B 双页+Tab / C 三页折衷 | **B 双页+Tab（修订）** | Dashboard 落地 + Configuration 配置中心（8 Tab：Settings/Subscriptions/Profiles/Override/YAML/Log/Updates/Backup），Backup 与 Subscriptions 收回到 Configuration 之内，菜单只保留 2 项 |
...
| Q10 | Backup 位置 | Updates Tab 下方 / 独立 Tab / 独立页 | **独立 Tab（修订）** | 与 Subscriptions 一同收回到 Configuration，菜单只保留 Dashboard + Configuration 两项；Backup 仍与三类资源更新解耦（独立 Tab）；保留可选 AES-256-CBC 加密 |
...
| Q21 | UI 框架 | 旧 raw-PHP / 全量 MVC / 混合 MVC | **混合 MVC（详见 1.8）** | 视觉/路由/i18n 严格遵循 helloworld + grids 示例；Settings/Subscriptions 数据进 OPNsense config.xml，YAML 文件态保留并由自定义控制器读写 |
...
└─────────────┼────────────────────────┼───────────────────────┘
              ...
              ▼                        ▼
       ┌──────────────┐  ┌────────────────────────────────────┐
       │ Mihomo API   │  │ /usr/local/etc/mihomo/             │
       │ 127.0.0.1:   │  │   base.yaml / override.yaml /      │
       │ 9090         │  │   profiles/ / subs.json /          │
       └──────────────┘  │   backups/                         │
                         └────────────────────────────────────┘
                                       │
                                       ▼
...
> 旧版"raw PHP under `www/`"模式废弃；本节给出向 OPNsense MVC（参考 helloworld / using_grids 示例）迁移后的标准布局。详细规范见 Section 1.8。

```
src/opnsense/mvc/app/
├── controllers/OPNsense/Mihomo/
│   ├── IndexController.php / DashboardController.php / ConfigurationController.php
│   ├── Api/SettingsController.php          # ApiMutableModelControllerBase（Settings 表单）
│   ├── Api/SubscriptionsController.php     # ApiMutableModelControllerBase（订阅 ArrayField CRUD）
│   ├── Api/OverrideController.php          # 自定义（override.yaml 文件）
│   ├── Api/ProfilesController.php          # 自定义（profiles/ 文件操作）
│   ├── Api/BackupController.php            # 自定义（tar.gz 打包/解包）
│   ├── Api/ServiceController.php           # 自定义（start/stop/restart/reconfigure 经 configd）
│   ├── Api/UpdateController.php            # 自定义（GitHub release 拉取 + 资源更新进度）
│   ├── Api/DashboardController.php         # 自定义（traffic/health/logs 转发 + 差分）
│   └── forms/
│       ├── general.xml / controller.xml / tun.xml / dns.xml / sniffer.xml / update.xml
│       └── dialogSubscription.xml
├── models/OPNsense/Mihomo/
│   ├── Mihomo.php / Mihomo.xml             # Settings + Subscription ArrayField 字段定义
│   ├── ACL/ACL.xml
│   └── Menu/Menu.xml
└── views/OPNsense/Mihomo/
    ├── dashboard.volt
    └── configuration.volt                  # 8 Tab：base_tabs_header + base_tabs_content + 各 Tab 内 partial
```

旧 `www/` 目录仅保留 302 跳转占位：

```
www/
├── services_mihomo.php   # 302 → /ui/mihomo/dashboard
└── sub.php               # 302 → /ui/mihomo/configuration#subscriptions
```
...
```
VPN > Mihomo
├── Dashboard               (/ui/mihomo/dashboard)
└── Configuration           (/ui/mihomo/configuration)
    ├── Settings            (Forms XML：general/controller/tun/dns/sniffer/update)
    ├── Subscriptions       (UIBootgrid + dialogSubscription.xml)
    ├── Profiles            (UIBootgrid，列表+激活+Refresh，profiles/*.yaml 文件)
    ├── Override            (YAML 编辑器，override.yaml 文件)
    ├── YAML                (当前激活 config.yaml 只读)
    ├── Log                 (mihomo.log 查看 + 清空)
    ├── Updates             (内核/GeoIP/UI 更新，UpdateController)
    └── Backup              (导入导出，BackupController)
```

菜单仅保留 2 个顶级条目：Dashboard 作为落地页，所有配置/数据源/备份操作收拢进 Configuration 的 8 个 Tab。Configuration 内 Tab 通过 `base_tabs_header` / `base_tabs_content` partial 渲染，URL hash 路由由 OPNsense Volt 框架原生支持。

菜单定义文件路径：`src/opnsense/mvc/app/models/OPNsense/Mihomo/Menu/Menu.xml`（不再用旧版 `menu/Magic/Menu/Menu.xml`）。
...
遵循 OPNsense 标准的 gettext 国际化（对齐 helloworld 示例 "Multi language / Translations" 节）：
- **Volt 模板**：`{{ lang._('your string here') }}`；含动态内容用 `{{ lang._('your %s here') | format(data) }}`
- **PHP 控制器/模型**：`gettext('...')`；动态内容 `sprintf(gettext('...'), $data)`
- **Forms XML**：`<label>` / `<help>` 由框架自动调用 gettext
- **Menu.xml**：`VisibleName` 由框架自动调用 gettext
- **ACL.xml**：`description` 由框架自动调用 gettext
- 翻译文件位于 `lang/` 目录：`lang/zh_CN/LC_MESSAGES/mihomo.po` → 编译为 `.mo`
- 先交付中文 `.po`，英文 `.pot` 模板同步生成供后续社区贡献
- **禁止拆分字符串**（OPNsense 翻译规范，避免句子被切碎）

### 涉及文件

| 文件 | 需翻译内容 |
|---|---|
| `mvc/app/views/OPNsense/Mihomo/*.volt` | 区块标题、按钮文字、说明文字、alert 消息（`{{ lang._('...') }}`） |
| `mvc/app/controllers/OPNsense/Mihomo/**/*.php` | 控制器内动态消息（`gettext('...')`） |
| `mvc/app/controllers/OPNsense/Mihomo/forms/*.xml` | Forms XML 中 `<label>` / `<help>`（框架自动） |
| `mvc/app/models/OPNsense/Mihomo/Menu/Menu.xml` | 菜单 `VisibleName`（框架自动） |
| `mvc/app/models/OPNsense/Mihomo/ACL/ACL.xml` | ACL `description`（框架自动） |

### 实施规则

- 在 `install.sh` / 插件 Makefile 部署时复制 `lang/zh_CN/LC_MESSAGES/mihomo.mo` 到 OPNsense 的 locale 路径
- OPNsense 框架自动初始化 gettext 与文本域绑定（基于插件名）；无需手写绑定
- Volt 模板中所有字面字符串走 `{{ lang._('...') }}`，PHP 控制器走 `gettext('...')`
- 复制 `.mo` 后重启 Web GUI 确保 gettext 缓存刷新
...
## 1.8 UI 风格对齐 OPNsense MVC 规范（NEW）

> 参考：<https://docs.opnsense.org/development/examples/helloworld.html> 与 <https://docs.opnsense.org/development/examples/using_grids.html>。本节为强制性结构基线，所有后续 Dashboard / Configuration / 子 Tab 设计均建立在这一框架之上。

### 1.8.1 决策（Q21）

| # | 议题 | 选项 | 选择 | 理由 |
|---|---|---|---|---|
| Q21 | UI 框架 | A 旧版 `www/*.php` + `guiconfig.inc` / B 全套 MVC（Volt + Forms XML + ApiMutable*）/ C 混合（MVC 表壳 + 自定义控制器读写 YAML 文件） | **C 混合** | Mihomo 状态主要在 YAML 文件而非 OPNsense config.xml，全量 MVC 不适配；但视觉/路由/i18n/表单/Grid 必须走官方 partial，否则与 OPNsense 视觉断裂 |

### 1.8.2 数据归位策略

| 数据 | 存储 | 控制器基类 | 模式 |
|---|---|---|---|
| Settings 表单字段（~40 个，原 base.yaml 表单管辖键） | OPNsense `config.xml`（Model XML 定义） | `ApiMutableModelControllerBase` | 标准 `settings/get` + `settings/set`，自动 Forms XML 渲染 |
| Subscriptions 条目（订阅源 CRUD） | OPNsense `config.xml`（`Subscription` ArrayField） | `ApiMutableModelControllerBase` | 标准 `searchItem` / `getItem` / `setItem` / `addItem` / `delItem` / `toggleItem` + UIBootgrid |
| Override / Profiles / 合成 config.yaml / 备份 tar.gz / 日志 | 文件系统（`/usr/local/etc/mihomo/`、`/var/log/`、`backups/`） | 自定义 `Api\ApiControllerBase` 子类 | 自定义 action：`getRaw` / `setRaw` / `validate` / `apply` / `download` / `restore` |
| Dashboard 实时数据（traffic / memory / connections / logs / health / update progress） | 转发 Mihomo `/api/...` + 临时文件 `/tmp/mihomo-*` | 自定义控制器 | `service/<name>Action`（GET / POST，纯 JSON） |
...
OPNsense config.xml ─┐
   (Settings model)  │
                     ├─► render base.yaml ─┐
                     │                     │
File: override.yaml ─┤                     ├─► merge ─► config.yaml ─► mihomo reload
                     │                     │
File: profile.yaml ──┘                     │
   (active.json 选定)                       │
```

> Settings Tab 保存时：`set` API 写 config.xml → 内部触发 `renderBaseYaml()` → `mergeAll()` → `atomicConfigUpdate()` → reload。订阅刷新走同一末端管道。

### 1.8.3 目录布局（MVC 标准）

旧版 raw-PHP 全部废弃，所有页面、API、模型迁移到 OPNsense MVC 路径：

```
src/opnsense/
├── mvc/app/
│   ├── controllers/OPNsense/Mihomo/
│   │   ├── IndexController.php             # 路由 /ui/mihomo/* → 渲染 Volt
│   │   ├── DashboardController.php
│   │   ├── ConfigurationController.php
│   │   ├── Api/
│   │   │   ├── SettingsController.php      # /api/mihomo/settings/* (ApiMutableModelControllerBase)
│   │   │   ├── SubscriptionsController.php # /api/mihomo/subscriptions/* (ArrayField CRUD)
│   │   │   ├── OverrideController.php      # /api/mihomo/override/get|set|validate (自定义 — YAML 文件)
│   │   │   ├── ProfilesController.php      # /api/mihomo/profiles/* (列表/激活/删除/refresh 文件操作)
│   │   │   ├── BackupController.php        # /api/mihomo/backup/export|import|list|restore
│   │   │   ├── ServiceController.php       # /api/mihomo/service/start|stop|restart|status|reconfigure
│   │   │   ├── UpdateController.php        # /api/mihomo/update/check|run/<core|geoip|ui>
│   │   │   └── DashboardController.php     # /api/mihomo/dashboard/traffic|health|logs
│   │   └── forms/
│   │       ├── general.xml                 # Settings Tab — Group A General
│   │       ├── controller.xml              # Settings Tab — Group B External Controller
│   │       ├── tun.xml                     # Settings Tab — Group C TUN
│   │       ├── dns.xml                     # Settings Tab — Group D DNS
│   │       ├── sniffer.xml                 # Settings Tab — Group E Sniffer
│   │       ├── update.xml                  # Settings Tab — Group F Auto Update
│   │       └── dialogSubscription.xml      # Subscriptions Tab 编辑对话框（grid_view 自动生成列）
│   ├── models/OPNsense/Mihomo/
│   │   ├── Mihomo.php                      # extends BaseModel
│   │   ├── Mihomo.xml                      # 字段定义（Settings 全部 + Subscription ArrayField）
│   │   ├── ACL/ACL.xml                     # 权限：page-mihomo-*
│   │   └── Menu/Menu.xml                   # 菜单（VPN > Mihomo > Dashboard | Configuration）
│   └── views/OPNsense/Mihomo/
│       ├── dashboard.volt                  # Dashboard 页（自定义卡片 + 仍引用 layout_partials）
│       └── configuration.volt              # Configuration 页（8 Tab：base_tabs_header + base_tabs_content）
├── scripts/mihomo/
│   ├── sub.sh / sub_cron.sh
│   ├── update_core.sh / update_geoip.sh / update_ui.sh
│   └── mihomo_health_check.sh
└── service/conf/actions.d/
    └── actions_mihomo.conf                 # configctl mihomo start|stop|restart|status|reconfigure
                                            # configctl mihomo sub-update
                                            # configctl mihomo update-core|update-geoip|update-ui
```

> **URL 变化**：所有页面从 `/mihomo_*.php` 迁移到 `/ui/mihomo/<page>`；所有 status/state 端点从 `/status_*.php` 迁移到 `/api/mihomo/...`。旧 URL 通过 `www/` 下的薄 PHP 跳转脚本做 302 兼容（见 Section 6.3）。

### 1.8.4 强制约定（视觉与交互）

1. **Volt 模板优先**：所有页面用 Volt，禁止再写 `head.inc` / `fbegin.inc` / `foot.inc` 直填 HTML。
2. **Forms XML 驱动**：Settings 表单与 Subscription 编辑对话框完全通过 `forms/*.xml` 定义 + `getForm("name")` 注入，不在 Volt 中手写 `<input>`。
3. **官方 partial**：
   - 单表单页：`{{ partial('layout_partials/base_form', {'fields': formXxx, 'id': 'frm_xxx'}) }}`
   - Tab 容器：`{{ partial('layout_partials/base_tabs_header', tabs) }}` + 各 Tab 内容 `{{ partial('layout_partials/base_tabs_content', tabs) }}`
   - 表格：`{{ partial('layout_partials/base_bootgrid_table', formGridXxx) }}`
   - 编辑对话框：`{{ partial('layout_partials/base_dialog', {'fields': formDialogXxx, 'id': formGridXxx['edit_dialog_id'], 'label': lang._('Edit ...')}) }}`
   - 应用按钮：`{{ partial('layout_partials/base_apply_button', {'data_endpoint': '/api/mihomo/service/reconfigure'}) }}`
4. **包装容器**：所有页面内容用 `<div class="content-box">` 包裹（OPNsense 标准卡片样式）；Tab 内部各自 `content-box`。
5. **JS 绑定**：表格用 `$("#tableId").UIBootgrid({...})`；apply 按钮用 `$("#reconfigureAct").SimpleActionButton()`；不引入 jQuery 之外的 JS 库（OPNsense 自带 jQuery）。
6. **i18n**：
   - Volt 模板：`{{ lang._('...') }}`
   - PHP 控制器：`gettext('...')`
   - Forms XML 中 `<label>`/`<help>` 自动走 gettext
   - Menu.xml `VisibleName` 通过翻译文件解析
   - 字符串不要拆分（OPNsense 翻译规范）
7. **ACL**：每页注册一个 ACL 节点（`page-mihomo-dashboard`、`page-mihomo-configuration`），用户组可按页粒度授权。
8. **configd 操作**：所有特权操作（启停服务、写 mihomo 配置、调用 sub.sh 等）通过 `configctl mihomo <action>`，PHP 控制器调用 `(new Backend())->configdRun("mihomo <action>")`，禁止 PHP 内直接 `exec("...")` 跑特权命令。
9. **API 响应规范**：成功 `{"status": "ok", ...}`，失败 `{"status": "failed", "message": "..."}`；批量 CRUD 返回 `{"result": "saved"}` / `{"result": "failed", "validations": {...}}`。沿用 OPNsense 框架默认返回结构。

### 1.8.5 与原三层合成模型的衔接

- `Mihomo.xml` Model 中 Settings 字段（Group A-F）保存到 OPNsense config.xml；保存后由 `SettingsController::setAction()` 末段调用 `Backend::configdRun("mihomo reconfigure")`。
- `reconfigure` action 在 `actions_mihomo.conf` 中绑定到 `/usr/local/opnsense/scripts/mihomo/reconfigure.py`，负责：读取 config.xml → 渲染 `base.yaml` → 调用 `mergeAll()` → 校验 → 原子替换 `config.yaml` → reload mihomo。
- `Override` / `Profiles` / `Backup` Tab 使用自定义控制器读写文件，但所有"写后重载"动作同样通过 `configctl mihomo reconfigure` 收口（保证文件锁与原子性单点）。

### 1.8.6 影响范围

- 旧 `www/services_mihomo.php` / `www/sub.php` 保留为最小化 302 跳转占位（不再含业务逻辑），分别跳转到 `/ui/mihomo/dashboard` 与 `/ui/mihomo/configuration#subscriptions`。
- `plugins/mihomo.inc` 保留（服务面板注册 + WAN IP 变化重启 + syslog 设施），无需迁入 MVC。
- 设计文档 Section 2/3/4/5 中所有"PHP 页面/文件"指代均按本节重新映射到 Volt + 控制器；旧 `mihomo_lib.inc.php` 公共库的角色由 `mvc/app/controllers/OPNsense/Mihomo/Api/Base.php`（自定义共享 Trait）承担。
- 设计文档 Section 6.3 旧 URL 跳转目标更新为新的 `/ui/mihomo/...` 路径。
...
Tab 导航，走 URL hash 保持刷新后当前 Tab。所有 Tab 实现在同一 `mihomo_configuration.php` 文件中：

- **Settings**（3.2）— base.yaml 表单
- **Subscriptions**（3.9，原 Section 5 内容下沉为 Tab）— 订阅源 CRUD + 抓取日志
- **Profiles**（3.4）— 列表/激活/Refresh
- **Override**（3.3）— override.yaml 编辑
- **YAML**（3.5）— 当前合成 config.yaml 只读
- **Log**（3.6）— mihomo.log 查看
- **Updates**（3.7）— 内核 / GeoIP / UI 在线更新
- **Backup**（3.10，原 Section 4 内容下沉为 Tab）— 导入/导出/恢复

UI 顺序遵循数据流：基础设施 → 数据源 → 生成的 profiles → 用户覆写 → 合成预览 → 运维 Tabs。
...
## 4. Backup Tab 详细设计（Configuration › Backup）

### 4.1 Tab 定位

Configuration 页第 8 个 Tab。用途：换机迁移、灾难恢复、版本回滚。仅备份用户数据，不含大文件（Country.mmdb / ui / 二进制）。原方案曾设为独立顶级页，现下沉为 Tab 以收紧菜单层级。
...
## 5. Subscriptions Tab 详细设计（Configuration › Subscriptions）

### 5.1 Tab 定位

Configuration 页第 2 个 Tab，紧跟 Settings。数据源管理：维护订阅源 + 触发抓取 + 查看抓取日志。与 Profile 一对一（默认每个订阅生成同名 profile，前缀 `sub-`）。原方案曾设为独立顶级页，现下沉为 Tab。
...
- 旧 URL `/services_mihomo.php` → 302 跳转 `/ui/mihomo/dashboard`
- 旧 URL `/sub.php` → 302 跳转 `/ui/mihomo/configuration#subscriptions`
- 旧 status 端点（`status_mihomo.php` / `status_mihomo_logs.php` / `status_sub_logs.php`）：迁入 MVC，由 `Api/DashboardController` 与 `Api/ServiceController` 接管；无对外路径承诺（前端已切到 `/api/mihomo/...`），旧文件不再保留
- `actions_mihomo.conf` 中 `[sub-update]` 动作名称不变，新增 `[reconfigure]`、`[update-core|update-geoip|update-ui]` 等动作
- Dashboard 页顶部增加**可关闭横幅**（dismissible）："UI 已升级至新版本，原配置已自动迁移。如有问题请前往 Configuration › Backup Tab 恢复。" 持续展示 2 周降低老用户困惑
